...
export function allocateContributions(
  ...
  const { annualContribution, mortgagePaidOff, year } = context
  ...
  if (year <= switchYears && mortgageBalance > 0) {
...
export function handleYearStart(
  state: StrategyState,
  context: StrategyContext,
  switchYears: number
): StrategyState {
  if (context.year <= switchYears || state.offsetBalance <= 0) {
    return state
  }
  ...
  return {
    ...state,
    offsetBalance: 0,
    investmentValue: state.investmentValue + state.offsetBalance,
    investmentCostBasis: state.investmentCostBasis + state.offsetBalance,
  }
}
